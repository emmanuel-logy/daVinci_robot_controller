...
> :information_source: In the above video, the PSM is moving along a predefined trajectory using the position controller. The desired positions for the predefined trajectory is generated by computing the Inverse Kinematics on MATLAB using the Damped-Least-Squares method. The model of the PSM (i.e. the DH parameters) were obtained from the AMBF simulator and loaded on MATLAB using the Peter Coorke's Robotics Toolbox. The desired joint positions were exported from MATLAB to a CSV file. Thereafter, those joint values were parsed in the python script and sent to the AMBF simulator. The communication between them is taking place using ROS. <br>
...
> :information_source: In the above video, the PSM is moving along a predefined trajectory using the position controller. The desired positions for the predefined trajectory is generated by computing the Inverse Kinematics on MATLAB using the Damped-Least-Squares method. The model of the PSM (i.e. the DH parameters) were obtained from the AMBF simulator and loaded on MATLAB using the Peter Coorke's Robotics Toolbox. <br>
